...
class TestAdminJobsCRUD:
    ...
    def test_job_tags_assignment(self, monkeypatch):
        ...
        # Create tag objects with explicit name attribute
        python_tag = Mock()
        python_tag.name = "Python"
        django_tag = Mock()
        django_tag.name = "Django"
        mock_tags = [python_tag, django_tag]
        
        mock_job.tags = mock_tags
        
        assert len(mock_job.tags) == 2
        assert mock_job.tags[0].name == "Python"
        assert mock_job.tags[1].name == "Django"
...
class TestAdminJobFiltering:
    ...
    def test_filter_by_tag(self):
        ...
        # Create tag mocks with proper name attribute
        python_tag1 = Mock(id=1)
        python_tag1.name = "Python"
        java_tag = Mock(id=2)
        java_tag.name = "Java"
        python_tag3 = Mock(id=1)
        python_tag3.name = "Python"
        
        jobs = [
            Mock(id=1, tags=[python_tag1]),
            Mock(id=2, tags=[java_tag]),
            Mock(id=3, tags=[python_tag3]),
        ]
